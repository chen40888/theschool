...
**
**
** IE8 only supports up to 32KB as embedded base64 images:
** http://stackoverflow.com/questions/1207190/embedding-base64-images
**/
class Minifier {
	const IMAGE_MAX_SIZE = 10; // Max image size in for inclusion | In KB
	...
	private
		...
		$gzip = false,
		$print = false,
		$timer = false, //For script execution time (src: http://bit.ly/18O3VWw)
		$use_closure = true, //Use google closure (utilizes cURL)
		$merge_images = false; //base64 images from CSS and include as part of the file?

	/**
	 * <p>Accepts the following options array:</p>
	 *
	 * <p>echo - Boolean | return the value or echo it | default: false</p>
	 * <p>encode - Boolean | true will base64 encode all CSS images and add them to the css.min.php file | default: false</p>
	 * <p>gzip - Boolean | output file as php with gzip | default: false</p>
	 *
	 * @access public
	 * @param array $vars
	 * @return mixed
	...
	public function __construct($vars = array()) {
		...
		if(isset($vars['encode']) && $vars['encode'] == true) {
			...
			//Log::w('Encode images into CSS as base64 data');
		}
	...
	private function make_min($src_file, $new_file, $do_minify = true) {
		...
		$this->prequel .= 'define("ROOT", realpath(dirname(dirname(dirname(__FILE__)))) . "/");' . PHP_EOL;
		$this->prequel .= 'include ROOT . "core/bootstrap.php";' . PHP_EOL;
		$this->prequel .= 'Conditional_Get::check(getlastmod(), true);' . PHP_EOL; // exits if client has cache
		...
		/**
		 * The .php extension is added automatically and must be accounted for to prevent files from being recreated
		 */
		...
		if(preg_match_all('/url\((["\']?)((?!["\']?data:).*?\.(gif|png|jpg|jpeg))\\1\)/i', $contents, $this->matches, PREG_SET_ORDER)) {
			...
			foreach($this->matches as $this->graphic) {
				...
				elseif(file_exists($this->directory . $this->graphic[2])) {
					...
					//Log::w('File exists locally | $size: ' . $this->filesize);
	 ...
	 **** files must all be the same type in order to prevent erroneous output contents (js and css do not mix)
	...
	public function merge($output_filename, $directory, $list_or_type = 'js', $exclude = array(), $order = array()) {
		...
		/**
		 * Reassign the $output_filename as we must account for the .php
		 * extension in order to prevent the file from being recreated
		 */
		if(strtolower(pathinfo($output_filename, PATHINFO_EXTENSION)) != '.php') {
			$output_filename .= '.php';
		}
		...
		if($this->create_new || !file_exists($output_filename)) {
			...
			//$do_minify = (conf('environment') != 'loc'); // Custom addition, bypass minifiying while developing on local machine
